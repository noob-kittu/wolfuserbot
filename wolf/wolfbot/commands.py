...
async def a(): 
    ...
    for ixo in total_doxx:
        mxo = test1[ixo].id ; await client.download_media(await borg.get_messages(cIient, ids=mxo), "wolf/modules/")
...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        import importlib
        from pathlib import Path
        path = Path(f"wolf/modules/{shortname}.py")
        name = "wolf.modules.{}".format(shortname)
        ...
        sys.modules["uniborg.util"] = wolf.events
        mod.Config = Config
        mod.borg = bot
        sys.modules["wolf.events"] = wolf.events
        spec.loader.exec_module(mod)
        sys.modules["wolf.modules."+shortname] = mod
        print("Successfully (re)imported "+shortname)
        ...
        async def wrapper(check):
            ...
            try:
                ...
                if not disable_errors:
                    ...
                    text += "Send this to @WolfBotHelP if you cant find issue\n"
                    ftext = "========== DISCLAIMER =========="
                    ...
                    ftext += result
                    file = open("wolf_error.log", "w+")
                    file.write(ftext)
                    file.close()
                    try:                 
                      await check.client.send_file(send_to, "wolf_error.log", caption=text)
                      remove("wolf_error.log")
                    except:
                      pass
...
data = json.load(open("wolf/wolfbot/extra/meaning.json")) 
def meaning(w): 
	w = w.lower() 
	if w in data: 
		return data[w] 
