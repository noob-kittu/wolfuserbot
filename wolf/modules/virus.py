...
@mrsammy(pattern=r"^\!scan(?: |$)(.*)", outgoing=True)
async def vt(event):
...
async def vt(event):
    ...
    vscan = downloaded_file_name
    if a ==2:
		      return await rkp.edit("`You need to Update wolfs to use this command.......`")
    if not vscan:
...
@mrsammy(outgoing=True, pattern="^!scan2(?: |$)(.*)")
async def _(event):
    ...
    async with bot.conversation(chat) as conv:
          try:        
              ...
              return await event.edit(f"**{WOLF_NNAME}:**  {song2.message}")
          except:      
              return await event.reply(f"Please unblock @DrWebBot and try again")

@wolfs.on(itzjass(pattern=f"scan2(?: |$)(.*)", allow_sudo=True))
async def _(event):
    reply_message = await event.get_reply_message() 
    if not reply_message or not event.reply_to_msg_id or not reply_message.media or not reply_message.media:
...
@mrsammy(outgoing=True, pattern="^!uscan(?: |$)(.*)")
async def _(event):
...
@wolfs.on(itzjass(pattern=f"uscan(?: |$)(.*)", allow_sudo=True))
async def _(event):
       rksong = event.pattern_match.group(1)
       if not rksong:
