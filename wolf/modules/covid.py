...
from wolf.wolfbot.commands import bot, itzjass, mrsammy
wolfs = bot


@mrsammy(outgoing=True, pattern="^!corona$")
async def iqless(e):
    await e.edit("Antivirus scan was completed \n⚠️ Warning! This  donkey has Corona Virus")      
    

    
@mrsammy(outgoing=True, pattern="^!covid (.*)")
...
@mrsammy(outgoing=True, pattern="^!covid2 (.*)")
async def _(event):
    if event.fwd_from:
        return 
...
@wolfs.on(itzjass(pattern=f"corona$", allow_sudo=True))
async def iqless(e):
    await e.reply("Antivirus scan was completed \n⚠️ Warning! This  donkey has Corona Virus")      
    

@wolfs.on(itzjass(pattern=f"covid (.*)", allow_sudo=True))
async def corona(event):
...
@wolfs.on(itzjass(pattern=f"covid2 (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return 
    input_str = event.pattern_match.group(1)
