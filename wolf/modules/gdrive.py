...
from wolf import (G_DRIVE_CLIENT_ID, G_DRIVE_CLIENT_SECRET,
...
async def show_current_gdrove_folder(event):
    if parent_id:
        ...
        await event.edit(
            f"My wolf is currently uploading files [here]({folder_link})")
    else:
        await event.edit(
            f"My wolf is currently uploading files to the root of my Google Drive storage.\
...
async def upload_file(http, file_path, file_name, mime_type, event, parent_id):
    ...
    body = {
        "title": file_name,
        "description": "Uploaded using PaperplaneExtended wolf",
        "mimeType": mime_type,
    }
    if parent_id:
