...
from wolf import CMD_HELP, ALIVE_NAME, PM_MESSAGE, WOLF_NAME, WOLF_MSG, ORI_MSG, bot, TEMP_DOWNLOAD_DIRECTORY, BOTLOG, BOTLOG_CHATID
...
from wolf.events import register
from telethon.tl.types import DocumentAttributeAudio
from wolf.wolfbot.commands import progress
from wolf.wolfbot.extra.google_images_download import googleimagesdownload
...
try:
   from wolf.wolfbot.commands import meaning
except:
   pass
...
@wolfs.on(itzjass(pattern=f"ggl (.*)", allow_sudo=True))
@mrsammy(outgoing=True, pattern="^!ggl (.*)")
...
@mrsammy(outgoing=True, pattern="^\!currency (.*)")
@wolfs.on(itzjass(pattern=f"currency (.*)", allow_sudo=True))
...
@mrsammy(outgoing=True, pattern="^!img (.*)")
@wolfs.on(itzjass(pattern=f"img (.*)", allow_sudo=True))
...
@wolfs.on(itzjass(pattern=f"movie (.*)", allow_sudo=True))
...
@wolfs.on(itzjass(pattern=f"google (.*)", allow_sudo=True))
...
@wolfs.on(itzjass(pattern=f"wiki (.*)", allow_sudo=True))
@mrsammy(outgoing=True, pattern=r"^\!wiki (.*)")
...
@wolfs.on(itzjass(pattern=f"ud (.*)", allow_sudo=True))
@mrsammy(outgoing=True, pattern="^\!ud (.*)")
...
@mrsammy(outgoing=True, pattern="^!magisk$")
@wolfs.on(itzjass(pattern=f"magisk$", allow_sudo=True))
...
@mrsammy(outgoing=True, pattern=r"^!specs(?: |)([\S]*)(?: |)([\s\S]*)")
@wolfs.on(itzjass(pattern=f"specs(?: |)([\S]*)(?: |)([\s\S]*)", allow_sudo=True))
async def devices_specifimions(request):
    lazy = request ; sender = await lazy.get_sender() ; me = await lazy.client.get_me()
    if not sender.id == me.id:
        rkp = await lazy.reply("`processing...`")
...
async def _(event):
    ...
    rkkk= await event.reply(f"`{WOLF_NNAME}: ` **Finding {rksong} in world wide please wait........**")
...
@wolfs.on(itzjass(pattern=f"mean(?: |$)(.*)", allow_sudo=True))
@mrsammy(outgoing=True, pattern="^!mean(?: |$)(.*)")
async def dic(event):
...
@mrsammy(outgoing=True, pattern=r"^\!reverse(?: |$)(\d*)")
@wolfs.on(itzjass(pattern=f"reverse(?: |$)(\d*)", allow_sudo=True))
