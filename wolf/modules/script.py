...
@mrsammy(outgoing=True, pattern="^\!term(?: |$|\n)([\s\S]*)")
async def evaluate(event):  
    await event.edit(f"**{WOLF_NNAME}**: `Running Terminal.....`")
    ...
    if not cmd:
        ...
        return
    if cmd in ("wolf.session", "env", "printenv"):
        return await event.edit(f"`{WOLF_NNAME}:` **Privacy Error, This command not permitted**")
    ...
    output.write(text)
    output.close()
    await event.client.send_file(event.chat_id, "term.txt", reply_to=event.id, caption=f"`{WOLF_NNAME}:` **Output too large, sending as file**")
    os.remove("term.txt")           
    return
...
async def evaluate(event):  
  if not FULL_SUDO:
      await query.reply(f"`{WOLF_NNAME}:` **Sorry , Normal Sudo cant acess this comand,  active advance sudo by set  FULL_SUDO as true in heroku var**") 
  else:
    ...
    if not cmd:
        ...
        return
    if cmd in ("wolf.session", "env", "printenv"):
        return await rkp.edit(f"`{WOLF_NNAME}:` **Privacy Error, This command not permitted**")
    process = await asyncio.create_subprocess_shell(
        cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
    running_processes.update({message: process})
    stdout, stderr = await process.communicate()
...
@mrsammy(outgoing=True, pattern=r"^\!exec(?: |$)([\s\S]*)")
async def run(run_q):
    code = run_q.pattern_match.group(1)
    
    if not code:
        await run_q.edit(f"`{WOLF_NNAME}:` **At least a variable is required to \
execute. Use !help script for an example.**")
        return

    if code in ("wolf.session", "env", "printenv"):
        await run_q.edit(f"`{WOLF_NNAME}:` **Privacy Error, This command not permitted**")
        return
...
@wolfs.on(itzjass(pattern=f"exec(?: |$)([\s\S]*)", allow_sudo=True))
async def run(run_q):
  if not FULL_SUDO:
      await query.reply(f"`{WOLF_NNAME}:` **Sorry , Normal Sudo cant acess this comand,  active advance sudo by set  FULL_SUDO as true in heroku var**") 
  else:
    ...
    if not code:
        await run_q.reply(f"`{WOLF_NNAME}:` **At least a variable is required to \
execute. Use !help script for an example.**")
        return

    if code in ("wolf.session", "env", "printenv"):
        await run_q.reply(f"`{WOLF_NNAME}:` **Privacy Error, This command not permitted**")
        return
...
@mrsammy(outgoing=True, pattern="^\!eval(?: |$)(.*)")
async def evaluate(query):
    if query.pattern_match.group(1):
        expression = query.pattern_match.group(1)
    else:
        await query.edit(f"`{WOLF_NNAME}:` **Please give command type !help script for more info**")
        return

    if expression in ("wolf.session", "env", "printenv"):
        await query.edit(f"`{WOLF_NNAME}:` **Privacy Error, This command not permitted**")
        return
...
@wolfs.on(itzjass(pattern=f"eval(?: |$)(.*)", allow_sudo=True))
async def evaluate(query):
  if not FULL_SUDO:
      await query.reply(f"`{WOLF_NNAME}:` **Sorry , Normal Sudo cant acess this comand,  active advance sudo by set  FULL_SUDO as true in heroku var**") 
  else:
    if query.pattern_match.group(1):
        ...
        await query.reply(f"`{WOLF_NNAME}:` **Please give command type !help script for more info**")
        return

    if expression in ("wolf.session", "env", "printenv"):
        await query.reply(f"`{WOLF_NNAME}:` **Privacy Error, This command not permitted**")
        return
...
@mrsammy(outgoing=True, pattern="^\!hash (.*)")
...
@wolfs.on(itzjass(pattern=f"hash (.*)", allow_sudo=True))
async def gethash(hash_q):
    hashtxt_ = hash_q.pattern_match.group(1)
    hashtxt = open("hashdis.txt", "w+")
    hashtxt.write(hashtxt_)
...
@mrsammy(outgoing=True, pattern="^\!base64 (en|de) (.*)")
