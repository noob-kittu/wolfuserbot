...
@mrsammy(pattern=r"^!decodeqr$", outgoing=True)
...
@mrsammy(pattern=r"!barcode(?: |$)([\s\S]*)", outgoing=True)
async def bq(event):
...
@wolfs.on(itzjass(pattern=f"barcode(?: |$)([\s\S]*)", allow_sudo=True))
async def bq(event):
    """ For .barcode command, genrate a barcode containing the given content. """    
    input_str = event.pattern_match.group(1)
    message = "SYNTAX: `.barcode <long text to include>`"
...
@mrsammy(pattern=r"!qrcode(?: |$)([\s\S]*)", outgoing=True)
...
@wolfs.on(itzjass(pattern=f"qrcode(?: |$)([\s\S]*)", allow_sudo=True))
async def make_qr(makeqr):
    """ For .makeqr command, make a QR Code containing the given content. """
    input_str = makeqr.pattern_match.group(1)
    message = "SYNTAX: `.makeqr <long text to include>`"
