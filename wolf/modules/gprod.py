...
from wolf.utils import register, errors_handler
...
async def get_full_user(event):  
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        try:
            ...
            return await event.edit("Error... Please report at @wolf_suPPorT", str(err))           
...
async def gben(wolf):
    ...
    if wolf.is_private:
        ...
        wolf.chat.title
    try:
        user, rank = await get_full_user(wolf)
    except:
        pass
    if me == user:
    ...
    if user:
        telchanel = [d.entity.id
                     for d in await wolf.client.get_dialogs()
        ...
        for x in telchanel:
          try:
             await wolf.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
