...
from wolf import AFKREASON, COUNT_MSG, CMD_HELP, ISAFK, BOTLOG, BOTLOG_CHATID, USERS, PM_AUTO_BAN, bot, TEMP_DOWNLOAD_DIRECTORY, LOGS
from wolf.events import mrsammy, kittu, itzjass
from telethon.errors import rpcbaseerrors
from wolf import BOTLOG, BOTLOG_CHATID, CMD_HELP
from wolf import CMD_HELP, BOTLOG_CHATID
...
from wolf import ALIVE_NAME, PM_MESSAGE, WOLF_NAME, WOLF_MSG, ORI_MSG, AFK_MESSAGE, AFK_MSG, BLOCK_MSG, BLOCK_MESSAGE
...
from wolf import (COUNT_PM, CMD_HELP, BOTLOG, BOTLOG_CHATID, PM_AUTO_BAN,LASTMSG, LOGS)
 ...
 async def permitpm(event):    
    if PM_AUTO_BAN:
        ...
        if event.is_private and event.chat_id != 710844948 and event.chat_id != self_user.id and not (
            ...
            try:
                from wolf.modules.sql_helper.pm_permit_sql import is_approved
                from wolf.modules.sql_helper.globals import gvarstatus
            ...
            if not apprv and event.text != UNAPPROVED_MSG:
                ...
                if COUNT_PM[event.chat_id] == 3:
                    await event.respond(
                             f"`{WOLF_NNAME}`: ** Dont spam my master's pm this is your last warning!!**")
                if COUNT_PM[event.chat_id] > 3:
                    await event.respond(
                             f"`{WOLF_NNAME}`: ** {BLOCK_MMSG} **")                        
                    try:
...
if PM_AUTO_BAN:
 @kittu(disable_edited=True, outgoing=True, disable_errors=True)
 async def auto_accept(event):    
    ...
    if event.is_private and event.chat_id != 777000 and event.chat_id != self_user.id and not (
        ...
        try:
            from wolf.modules.sql_helper.pm_permit_sql import is_approved
            from wolf.modules.sql_helper.pm_permit_sql import approve
        except AttributeError:
            return
...
@mrsammy(outgoing=True, pattern="^\!notifoff$")
async def notifoff(noff_event):  
    try:
        from wolf.modules.sql_helper.globals import addgvar
    except AttributeError:
...
@mrsammy(outgoing=True, pattern="^\!notifon$")
async def notifon(non_event):    
    try:
        from wolf.modules.sql_helper.globals import delgvar
    except AttributeError:
...
@mrsammy(outgoing=True, pattern="^\!allow$")
...
@mrsammy(outgoing=True, pattern="^\!disallow$")
async def disapprovepm(disapprvpm):
    try:
        from wolf.modules.sql_helper.pm_permit_sql import dissprove
...
@mrsammy(outgoing=True, pattern="^\!block$")
async def blockpm(block):
    ...
    if block.reply_to_msg_id:
        ...
        await block.edit(f"`{WOLF_NNAME}: You've been blocked!`")
        uid = ruser.id
    else:
...
@mrsammy(outgoing=True, pattern="^\!unblock$")
async def unblockpm(unblock):
    ...
    if unblock.reply_to_msg_id:
        ...
        await unblock.edit(f"`{WOLF_NNAME}: You have been unblocked.`")
    if BOTLOG:
...
try:
    from wolf.modules.sql_helper.globals import gvarstatus, addgvar, delgvar
    afk_db = True
except AttributeError:
    afk_db = False
AFKSTR = [f"`{WOLF_NNAME}:` ** {AFK_MMSG} **"]
global USER_AFK  
global afk_time  
global afk_start
global afk_end
...
@mrsammy(outgoing=True, pattern="^!afk(?: |$)(.*)", disable_errors=True)
...
@mrsammy(incoming=True, disable_edited=True)
  ...
  if not mention.chat_id in W_CHAT:   
    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:        
            ...
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"`{WOLF_NNAME}:`**{AFK_MMSG}**\
                    \n\n`Reason:` **{AFKREASON}**\n`Since :` **{afk_since}**")
                else:
    ...
    if sender.is_private and sender.sender_id != 710844948 and not (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(f"`{WOLF_NNAME}:`**{AFK_MMSG}**\
                    \n\n`Reason:` **{AFKREASON}**\n`Since :`**{afk_since}**")
                else:
...
async def _(event):
    ...
    spamw =  "[Add Apikey](https://t.me/mrsammy/157)" ; sreason = {}
    try:
...
@mrsammy(pattern="^\!whois(?: |$)(.*)", outgoing=True)
@wolfs.on(itzjass(pattern=f"whois(?: |$)(.*)", allow_sudo=True))
async def who(event):
    sender = await event.get_sender() ; me = await event.client.get_me()
    if not sender.id == me.id:
        rkp = await event.reply("`processing`")
...
@mrsammy(outgoing=True, pattern="^\!purge$")
@wolfs.on(itzjass(pattern=f"purge$", allow_sudo=True))
...
@mrsammy(outgoing=True, pattern="^\!purgeme")
@wolfs.on(itzjass(pattern=f"purgeme", allow_sudo=True))
...
@mrsammy(outgoing=True, pattern="^\!del$")
@wolfs.on(itzjass(pattern=f"del$", allow_sudo=True))
...
@mrsammy(outgoing=True, pattern="^\!edit")
...
@mrsammy(outgoing=True, pattern="^\!sd")
