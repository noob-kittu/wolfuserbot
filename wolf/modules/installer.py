...
@sammy(pattern="^!install", outgoing=True)
    ...
    if event.reply_to_msg_id:
        try:
            downloaded_file_name = await event.client.download_media(  # pylint:disable=E0602
                ...
                "/root/wolf/wolf/modules/"  # pylint:disable=E0602
            )
...
@sammy(pattern="^!load (?P<shortname>\w+)$", outgoing=True)
...
@sammy(pattern="^!send (?P<shortname>\w+)$", outgoing=True)
async def send(event):
    if event.fwd_from:
        return
    message_id = event.message.id
    input_str = event.pattern_match["shortname"]
    the_plugin_file = "./wolf/modules/{}.py".format(input_str)
    start = datetime.now()
...
@sammy(pattern="^!unload (?P<shortname>\w+)$", outgoing=True)
...
@mrsammy(outgoing=True, pattern="^!installall (.*)")
async def install(event):
    try:
    ...
    for ixo in total_doxx:
        await event.edit(text) ; mxo = cui[ixo].id ; downloaded_file_name = await event.client.download_media(await client.get_messages(chat, ids=mxo), "wolf/modules/")
        if "(" not in downloaded_file_name:
